...
def login_require(onlyfilterpost=False):
    def _wrap(func):
        @functools.wraps(func)
        def __wrap(*args,**kw):
            print(onlyfilterpost)
            print(args[0].META.get('REQUEST_METHOD'))
            if onlyfilterpost:
                if not args[0].user.is_authenticated() and args[0].META.get('REQUEST_METHOD','GET')=='POST':#第0个一定是request
